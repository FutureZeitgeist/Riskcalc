...
# Roblox Extension Import
from fair_monte_carlo.models.roblox_financial_impact import RobloxFinancialImpact

app = Flask(__name__)

# Global Toggle for Advanced GRC Features
ENABLE_ROBLOX_EXTENSIONS = True
...
def simulate():
    ...
    try:
        ...
        # --- START ROBLOX EXTENSION LOGIC ---
        roblox_secondary_impact = 0
        if ENABLE_ROBLOX_EXTENSIONS:
            # Extract dynamic monetization variables from payload
            daus = float(data.get('daus', 0))
            abpdau = float(data.get('abpdau', 0))
            month = data.get('month', 1)
            cohort = data.get('cohort', '18-34')
            gender = data.get('gender', 'mixed')
            
            # Initialize specialized engine
            engine = RobloxFinancialImpact(daus, abpdau, month, cohort, gender)
            # Calculate demographic-specific secondary churn loss
            roblox_secondary_impact = engine.calculate_secondary_churn_loss(primary_loss_event=0)
        # --- END ROBLOX EXTENSION LOGIC ---
        ...
        response = {
            ...
            "summary": {
                ...
                # Inject Roblox specialized secondary loss into summary
                "roblox_secondary_loss": float(roblox_secondary_impact)
            },
            "mode": "ROBLOX_ADVANCED" if ENABLE_ROBLOX_EXTENSIONS else "STANDARD",
            "lef": {
...
def update_scenario(scenario_id):
    ...
    try:
        ...
        if "lef_config" in data: scenario.lef_config = data["lef_config"]
        if "lm_config" in data: scenario.lm_config = data["lm_config"]
        if "iterations" in data: scenario.iterations = data["iterations"]
        db.session.commit()
        return jsonify({
            "success": True,
            "scenario": scenario.to_dict()
